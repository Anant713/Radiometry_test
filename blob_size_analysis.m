...
%% ========== USER PARAMETERS ==========
inputFolder = sprintf('C:\\Users\\Anant\\OneDrive\\Desktop\\vbn_data\\vbn_data\\blob_size_data');
...
CROP_PAD = 3;      % padding around each detected blob when fitting
...
results = [];
header = {'filename','led_role','gauss_x','gauss_y','sigma_x','sigma_y','FWHM_px','centroid_x','centroid_y','peak of gauss','blob area','centroid_to_gauss_dist','dist bw leds'};
...
for fi = 1:numel(files)
    ...
    for r = 1:1
        ...
        ux = fits(idxs(r+1)).centroid(1);
        uy = fits(idxs(r+1)).centroid(2);
        peak = fits(idxk).amp;
        dist_cg = sqrt( (cx - gauss_x)^2 + (cy - gauss_y)^2 ); 
        dist_bw_led = sqrt ( (cx - ux)^2 + (cy - uy)^2 ) ; 
        MEAN_CENTROID_ERROR= MEAN_CENTROID_ERROR + dist_cg/60;
        mean_gauss_peak_val = peak + mean_gauss_peak_val;
        % append to results
        results = [results; {files(fi).name, roles{r}, gauss_x, gauss_y, sx, sy, FWHM_px, cx, cy,peak,blobs(idxCentral).area, dist_cg,dist_bw_led}];
    end
